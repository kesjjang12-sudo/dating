...
export interface Section { key: string; label: string; title: string; paras: string[]; }
...
const DM: Record<number, { nick: string; who: string; love: string }> = {
  ...
  3: { nick: '화롯불', who: '정화(丁火)는 태양이 아니라 촛불·화롯불입니다. 은근하고 오래 타는 온기, 한 곳을 깊이 비추는 집중력이 있어요. 겉은 순해 보이지만 심지가 있고, 어두운 곳에서 더 귀해지는 불이라 어려운 시절에 진가가 드러납니다. 세심하고 예민하며 정신적인 것에 끌립니다.', love: '티 안 나게 오래 좋아하고, 말은 늦습니다. 한번 마음을 주면 헌신적이고 관계가 끝나도 오래 남아요. 상대가 마음을 모르는 게 이 사주의 연애 문제 — 표현을 연습해야 합니다.' },
  4: { nick: '큰 산', who: '무토(戊土)는 큰 산, 넓은 들입니다. 묵직하고 신용이 있고, 한번 맡은 건 끝까지 책임집니다. 감정 기복이 적어 곁에 있으면 안정감을 주지만, 변화에 느리고 고집이 세며 속을 잘 안 보여요. 사람들이 기대는 자리에 서는 명식입니다.', love: '느리게 시작해서 오래 갑니다. 화려한 이벤트보다 꾸준함으로 사랑을 증명하는 유형이에요. 표현이 적어 답답하다는 소리를 듣지만, 떠나지 않는 것이 이 사주의 사랑법입니다.' },
  5: { nick: '기름진 밭', who: '기토(己土)는 만물을 기르는 밭의 흙입니다. 포용력이 있고 남을 돌보는 데 재능이 있으며, 실속을 챙기면서도 티를 내지 않아요. 겉은 순하지만 안에 계산이 서 있고, 한번 마음이 떠나면 조용히 정리합니다. 살림·경영·교육처럼 무언가를 키우는 일에 강합니다.', love: '상대를 잘 챙기고 잘 받아 줍니다. 헌신적인데, 그 헌신을 당연하게 여기는 상대에게는 상처를 오래 품어요. 말 대신 행동으로 사랑하는 유형입니다.' },
  6: { nick: '무쇠', who: '경금(庚金)은 아직 다듬어지지 않은 무쇠·바위입니다. 결단이 빠르고 의리가 있으며, 맺고 끊는 게 분명합니다. 정의감이 강해 불의를 보면 나서고, 그만큼 적도 생겨요. 다듬어질수록(고생할수록) 빛나는 명식이라 젊어서의 단련이 훗날의 무기가 됩니다.', love: '좋고 싫음이 분명하고, 사귀면 의리로 지킵니다. 감정 표현은 무뚝뚝하지만 행동은 확실해요. 자존심이 세서 먼저 사과하기 어려운 게 약점입니다.' },
  7: { nick: '보석', who: '신금(辛金)은 이미 세공이 끝난 보석·장신구의 금입니다. 섬세하고 눈썰미가 예리하며 자기 기준이 분명해요. 남들은 까다롭다 하지만 본인은 정확한 겁니다. 겉은 부드러운데 속에 칼날이 있고, 그 칼날은 자기 자신에게 더 자주 향합니다. 알아봐 주는 사람 앞에서 빛나고, 함부로 다루는 사람에겐 차갑게 식습니다.', love: '끌어당기되 쉽게 열지 않습니다. 취향이 확실해 아무나 만나지 않고, 사귀면 뜨겁고 헤어지면 매섭게 정리해요. 상대의 허점을 정확히 짚는 말이 나오는 게 이 사주의 연애 약점입니다.' },
  8: { nick: '큰 바다', who: '임수(壬水)는 큰 강, 바다입니다. 그릇이 크고 생각이 깊으며, 겉은 잔잔해도 속으론 끊임없이 움직입니다. 지혜와 융통성이 있고 사람을 가리지 않지만, 방향이 정해지지 않으면 넘치거나 흩어지기 쉬워요. 자유를 중시하고 얽매이는 걸 싫어합니다.', love: '깊고 넓게 사랑하지만 한 곳에 오래 머무는 걸 스스로 의심하는 별입니다. 정신적 교류가 되는 상대에게 끌리고, 구속하는 상대에게선 빠져나갑니다.' },
  9: { nick: '이슬비', who: '계수(癸水)는 이슬, 안개, 조용히 스미는 비입니다. 감수성이 예민하고 직관이 뛰어나며, 조용히 상황을 읽어 낸 뒤 스며들듯 움직입니다. 눈에 띄지 않게 일을 이루는 타입이고, 속으로 생각이 아주 많아요. 예술·연구·상담처럼 깊이가 필요한 일에 어울립니다.', love: '섬세하고 감정 이입이 깊어 상대의 기분을 먼저 읽습니다. 다만 상처도 깊게 받고 혼자 삭이는 편이라 — 마음을 말로 꺼내 놓는 상대를 만나야 편해집니다.' },
};

const GROUP_TEXT: Record<Group, string> = {
  비겁: '비겁(자존·독립)의 기운이 두드러집니다. 제 밥그릇은 제가 챙기고, 남에게 기대는 걸 어려워하며, 경쟁 상황에서 오히려 힘이 나는 사람이에요.',
  식상: '식상(표현·창조)의 기운이 두드러집니다. 말·글·손끝·아이디어로 자기를 드러내는 사람이고, 담아두면 병이 되고 꺼내놓으면 복이 되는 유형입니다.',
  재성: '재성(현실·재물)의 기운이 두드러집니다. 돈의 흐름과 현실 감각이 좋고 결과로 말하는 사람이에요. 다만 욕심보다 체력이 먼저 바닥나는 재다신약 경향을 살펴야 합니다.',
  관성: '관성(책임·규범)의 기운이 두드러집니다. 예의 바르고 참을성이 많고 조직에서 신뢰를 얻는 사람인데, 그만큼 눌리는 것도 많아 혼자 다 짊어지면 탈이 납니다.',
  인성: '인성(사고·배움)의 기운이 두드러집니다. 배운 것으로 서고 아는 것으로 신뢰를 얻는 사람이에요. 생각이 많아 결정이 느리지만 한번 내리면 잘 바꾸지 않습니다.',
};
...
const spouseHouseText = (pi: PillarInfo, gender: 'M' | 'F'): string => {
  ...
  const g = GROUP_OF[s];
  let t = `배우자궁(일지)에 ${s}${GA(s)} 앉아 있고, 십이운성으로는 ${st}입니다. `;
...
function themSections(a: PersonAnalysis, dae: LuckNow | null): Section[] {
  const dm = DM[a.dayStem];
  const dominant = (Object.entries(a.groupCount) as [Group, number][]).sort((x, y) => y[1] - x[1])[0];
  const spouseEl = a.gender === 'M' ? CTRL[a.element] : CTRL_BY[a.element];
  ...
  const temperament: string[] = [
    `${a.name}님의 일간은 ${STEMS_KO[a.dayStem]}${a.element}(${STEMS_HANJA[a.dayStem]}) — ${a.nick}입니다. ${dm.who}`,
    `월지가 ${BRANCHES_KO[a.pillars[2].p!.branch]}(${seasonEl})${IRA(seasonEl)} ${a.gyeokguk}을 이루고, 명식 전체로는 ${a.weak ? '신약(身弱)' : '신강(身强)'}에 속합니다. ${a.weak ? `힘을 보태 주는 ${a.favorable.join('·')} 기운을 반기고, ${a.avoid} 기운이 더 오면 부담이 됩니다. 혼자 다 짊어지는 것보다 사람과 배움에 기대는 편이 이 사주의 살길이에요.` : `기운이 넉넉해 ${a.favorable.join('·')} 기운으로 풀어내야 빛나는 사주입니다. 가진 힘을 쓸 곳(일·표현·책임)이 있어야 편안해져요.`}`,
  ...
  if (romance.length) love.push(`${romance.map((b) => `${b.label}(${b.where})`).join('·')}이 있어 굳이 애쓰지 않아도 주변에서 먼저 다가오는 유형입니다. 첫인상에서 "뭔가 있다"는 소리를 듣는 사주예요.`);
  else love.push('도화·홍염처럼 스스로 매력을 뿜는 살은 없어, 첫눈에 띄는 타입보다 알고 보면 좋은 사람형입니다. 시간을 함께 보낼수록 진가가 드러나요.');
  if (gm) love.push(`${gm.label.replace('주', '지')} ${BRANCHES_KO[gm.p!.branch]}가 공망(空亡)입니다. 공망은 "자리는 있는데 비어 있다"는 뜻 — ${gm.label === '시주' ? '말년·자식 자리가 비어 정신적 가치를 좇는 쪽으로 읽고, ' : ''}인연에서는 다가오는 사람은 많은데 정작 마음이 채워지는 사람은 드문 시기가 있을 수 있습니다.`);
  ...
  if (dae) marriage.push(dae.marriage
    ? `지금 대운(${pillarKo(dae.pillar)}, ${dae.startAge}세~)은 ${dae.stemS}·${dae.branchS} — ${a.gender === 'M' ? '재성(아내의 별)' : '관성(남편의 별)'}이 대운으로 들어온 인연 대운입니다. 결혼 인연이 구체적인 모양을 갖추는 10년이에요.`
    : `지금 대운(${pillarKo(dae.pillar)}, ${dae.startAge}세~)은 ${dae.stemS}·${dae.branchS}로 배우자 별이 직접 들어온 대운은 아닙니다. 인연은 대운보다 본인의 선택과 세운(한 해 운)에 좌우되는 시기예요.`);

  const work: string[] = [];
  if (hasFood && hasWealth) work.push('식상과 재성이 함께 있어 식상생재(食傷生財) — 내가 만들어 낸 결과물·기술·아이디어가 재물로 이어지는 구조입니다. 월급형보다 자기 이름으로 하는 일에서 수입이 커지는 유형이에요.');
  ...
  if (a.weak && a.groupCount.재성 >= 2) work.push('재다신약(財多身弱) — 돈 벌 기회는 보이는데 몸이 못 따라가는 순간이 옵니다. 욕심이 아니라 체력의 문제라는 걸 알아 두고, 여러 갈래로 벌리지 말고 하나에 집중하는 것이 이 사주의 재물 전략입니다.');
  const bh = a.badges.find((b) => b.label === '백호대살');
  if (bh) work.push(`${bh.where} 백호대살은 강한 기운이 든 것입니다. 옛 해석은 사고·수술 조심이지만, 현대 명리에서는 피·칼·돈·법을 다루는 일(의료·법률·금융·기술·요리)에서 오히려 무기가 되는 살로 봅니다.`);
  const hg = a.badges.find((b) => b.label === '화개살');
  if (hg) work.push('화개살은 예술·철학·연구처럼 혼자 깊이 파는 재능의 별입니다. 사람 많은 자리보다 몰입할 때 결과가 나오는 유형이에요.');
  const mc = a.badges.find((b) => b.label === '문창귀인');
  if (mc) work.push(`${mc.where}의 문창귀인은 글·기록·정리·학문의 별 — 문서와 지식으로 신뢰를 얻는 재능이 있습니다.`);
...
function coupleSections(me: PersonAnalysis, them: PersonAnalysis, rel: Relation[], c: CompatResult): Section[] {
  ...
  if (r === 'a-controls-b') structure.push(`${man.name}님의 ${man.element}${GA(man.element)} ${woman.name}님의 ${woman.element}${EUL(woman.element)} 극합니다. 그런데 명리에서 남자에게 "내가 극하는 오행"은 재성 곧 아내의 별이고, 여자에게 "나를 극하는 오행"은 관성 곧 남편의 별이에요. 이 두 사람의 극은 다투는 공식이 아니라 남녀 인연의 공식 그 자체입니다. ${samePol ? '둘 다 음양이 같아 편재·편관의 만남 — 정재·정관처럼 잔잔하기보다 끌림이 선명하고 자극적인 쪽입니다.' : '음양이 달라 정재·정관의 만남 — 명리에서 가장 반듯하고 안정적인 부부 인연의 배열로 봅니다.'}`);
  else if (r === 'b-controls-a') structure.push(`${woman.name}님의 ${woman.element}${GA(woman.element)} ${man.name}님의 ${man.element}${EUL(man.element)} 극합니다. 남자에게 상대는 관성(規範·책임), 여자에게 상대는 재성(내가 다루는 것)이에요. 전통적인 남녀 배열과는 반대라 — 여자가 관계의 방향을 잡고 남자가 배우자를 통해 성장하는 구조입니다. 편안한 안정형보다 서로를 키우는 성장형 인연으로 봅니다.`);
  else if (r === 'same') structure.push(`두 사람의 일간이 같은 ${man.element} 기운입니다. 비화(比和)의 만남 — 설명 없이 통하는 편안함, 친구 같은 연인이에요. 닮은 만큼 부딪히는 지점도 닮았고, 주도권을 두고 은근한 경쟁이 생길 수 있습니다.`);
  else if (r === 'a-gives-b') structure.push(`${man.name}님의 ${man.element}${GA(man.element)} ${woman.name}님의 ${woman.element}${EUL(woman.element)} 생합니다. 남자에게 상대는 식상(아끼고 표현하는 대상), 여자에게 상대는 인성(든든한 뒷배)이에요. 남자가 주고 여자가 받는 보호자 구조 — 남자는 아낌없이 주게 되고, 여자는 곁에 있는 것만으로 힘을 받습니다. 받는 법도 함께 연습하면 좋아요.`);
  else structure.push(`${woman.name}님의 ${woman.element}${GA(woman.element)} ${man.name}님의 ${man.element}${EUL(man.element)} 생합니다. 여자에게 상대는 식상(아끼는 대상), 남자에게 상대는 인성(나를 키우는 힘)이에요. 여자가 주고 남자가 받는 구조 — 남자는 이 관계에서 배우고 안정되고, 여자는 돌보는 기쁨이 있되 지치지 않도록 균형이 필요합니다.`);

  // 서로 채워 주는 오행
  const fill: string[] = [];
  const supply = (x: PersonAnalysis, y: PersonAnalysis) => {
    const needs = x.favorable.filter((e) => x.elementCount[e] <= 1 && y.elementCount[e] >= 2);
    return needs;
  };
  ...
  if (meGets.length && themGets.length) fill.push('각자의 용신(가장 필요한 기운)을 상대가 들고 온 배열입니다. 옛 어른들이 "제 짝은 제 약을 들고 온다" 한 것이 이런 경우예요.');
  if (!fill.length) fill.push('서로의 용신을 직접 채워 주는 배열은 아닙니다. 기운의 보완보다는 취향·가치관·시기의 조율로 만들어 가는 인연이에요 — 그만큼 노력이 곧바로 관계에 반영됩니다.');
  ...
  if (talker && holder && talker !== holder) personality.push(`${talker.name}님은 식상의 사람 — 틀린 걸 보면 말합니다. ${holder.name}님은 관성의 사람 — 불편해도 참습니다. 이 조합의 위험은 싸움이 아니라 한쪽만 말하고 한쪽은 쌓아 두는 것이에요. 겉으론 평화로운데 어느 날 ${holder.name}님이 조용히 등을 돌리는 그림이 최악의 시나리오, 반대로 ${talker.name}님의 정확함이 ${holder.name}님의 망설임을 끊어 주고 ${holder.name}님의 인내가 ${talker.name}님의 날을 감싸 주는 것이 최선입니다.`);
  else if (me.weak && them.weak) personality.push('두 사람 모두 신약한 명식입니다. 서로 기대는 관계가 되기 쉽고, 편안하지만 결정을 미루는 습관이 둘 다 있어 — 누가 먼저 결정하는지 규칙을 정해 두면 좋습니다.');
  else if (!me.weak && !them.weak) personality.push('두 사람 모두 신강한 명식입니다. 각자 주도권이 강해 부딪히면 크게 부딪히지만, 서로의 힘을 인정하면 가장 든든한 파트너가 됩니다. 영역을 나누는 것이 관건이에요.');
  else { const s = me.weak ? them : me, w = me.weak ? me : them; personality.push(`${s.name}님이 신강, ${w.name}님이 신약한 조합입니다. ${s.name}님이 이끌고 ${w.name}님이 받치는 구조가 자연스럽게 생기는데 — ${w.name}님의 의견이 묻히지 않도록 ${s.name}님이 한 박자 기다려 주는 것이 균형의 열쇠입니다.`); }
  ...
  if (haps.length) affection.push(`천간합이 ${haps.length}개(${haps.map((h) => h.kind).join('·')}) 있습니다. 천간합은 이유를 설명하기 어려운 호감의 자리 — 처음부터 편하고 자꾸 눈이 가는 근거예요.${haps.length >= 2 ? ' 합이 겹으로 들어 사랑의 온도가 높은 궁합입니다.' : ''}`);
  else affection.push('천간합은 없습니다. 첫눈에 확 끌리는 종류보다 함께 시간을 보내며 서서히 데워지는 종류의 애정이에요. 느린 대신 식는 것도 느립니다.');
  ...
  wedding.push(good >= 2 ? `지지에 육합·삼합이 ${good}개 — 큰 방향과 인생관에서 한 배를 타는 사람들입니다. 결혼 초보다 결혼 10년 후가 더 좋은, 오래 갈수록 붙는 궁합이에요.` : good === 1 ? '지지에 합이 하나 있어 큰 틀은 붙들려 있습니다. 결혼 궁합의 관건은 합이 아니라 아래 갈등 자리를 어떻게 다루는가예요.' : '지지에 합이 없어 "운명적으로 맞는다"는 종류의 궁합은 아닙니다. 대신 서로 다른 세계를 가진 사람들이라, 각자의 세계를 존중하면 오래 가는 결혼이 됩니다.');
  wedding.push(bad === 0 ? '충·원진이 없습니다. 부딪힘이 적은 편안한 결혼 궁합 — 대신 자극이 적어 서로에게 익숙해지는 속도도 빠르니, 함께 새로운 것을 하는 습관이 필요해요.' : `충·원진이 ${bad}개 있습니다. 명리에서 충(沖)은 깨지는 기운이 아니라 움직이는 기운 — ${good ? '합이 큰 틀을 붙들고 있으니 이 충들은 암초가 아니라 파도입니다.' : '합이 없는 상태의 충이라 파도가 곧바로 배에 닿습니다. 갈등 자리를 미리 알고 규칙을 정해 두는 것이 이 궁합의 결혼 준비예요.'}`);
  ...
  return [
    { key: 'structure', label: '인연의 구조', title: r === 'a-controls-b' ? '서로가 서로의 배우자 별' : r === 'same' ? '닮은 두 사람' : r.includes('gives') ? '주고 받는 구조' : '그녀가 이끄는 구조', paras: structure },
    { key: 'fill', label: '서로 채워 주는 것', title: fill.length >= 2 ? '제 짝은 제 약을 들고 온다' : '기운의 보완', paras: fill },
    { key: 'personality', label: '성격 궁합', title: talker && holder && talker !== holder ? '말하는 사람과 참는 사람' : '기질의 균형', paras: personality },
    { key: 'affection', label: '애정 궁합', title: haps.length ? `천간합 ${haps.length}개 — 이유 없이 끌리는 자리` : '천천히 데워지는 애정', paras: affection },
    { key: 'wedding', label: '결혼 궁합', title: good >= 2 ? '오래 갈수록 붙는 합' : '결혼 궁합', paras: wedding },
    { key: 'conflict', label: '갈등 포인트', title: warns.length ? `${warns.length}개의 자리를 알면 다툼의 8할이 설명된다` : '부딪힐 자리가 없는 궁합', paras: conflict },
  ];
}

// ── 시기 (대운·세운) ─────────────────────────────────────
function luckNow(a: PersonAnalysis, p: Person, birth: { y: number; m: number; d: number }, today: Date): LuckNow | null {
  const hb = p.pillars.hour ? p.pillars.hour.branch : null;
  const hh = hb === null ? 12 : (hb * 2) % 24;
  const dd = daeun(birth.y, birth.m, birth.d, hh, 0, p.gender, p.pillars);
  if (!dd) return null;
...
function timingSections(me: PersonAnalysis, them: PersonAnalysis, lm: LuckNow | null, lt: LuckNow | null, year: number, yp: Pillar): Section[] {
  ...
  if (lm?.marriage && lt?.marriage) dae.push('두 사람의 인연 대운이 지금 겹쳐 있습니다. 결혼 궁합에서 "좋은가"보다 중요한 것이 "때가 맞는가"인데 — 두 분은 때가 맞습니다.');
  else if (lm?.marriage || lt?.marriage) dae.push(`${lm?.marriage ? me.name : them.name}님 쪽은 인연 대운, 다른 한쪽은 아닙니다. 한쪽이 서두르고 한쪽이 느긋해지기 쉬운 배열이라 속도 차이를 서로 인정해야 해요.`);
  else dae.push('두 사람 모두 배우자 별이 대운으로 들어온 시기는 아닙니다. 지금은 관계를 다지는 시기이고, 결정은 세운이 받쳐 주는 해를 고르는 것이 좋습니다.');
  out.push({ key: 'daeun', label: '대운 — 10년의 계절', title: lm?.marriage && lt?.marriage ? '두 사람의 인연 대운이 겹쳐 있다' : '지금 두 사람이 서 있는 계절', paras: dae });

  const se: string[] = [];
  for (const a of [me, them]) {
  ...
  out.push({ key: 'seun', label: `${year}년 세운`, title: `${pillarKo(yp)}년, 두 사람의 올해`, paras: se });
  return out;
}
...
export function fullReading(me: Person & { birth: BirthInput }, them: Person & { birth: BirthInput }, today = new Date()): FullReading {
  ...
  return {
    ...
    themSections: themSections(B, lt),
